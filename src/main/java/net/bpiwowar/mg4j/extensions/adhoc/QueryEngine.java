...
public class QueryEngine implements FlyweightPrototype<QueryEngine> {
     ...
     * @param onlyDocuments Restricts the documents to a given set
     * @param results       an array list that will hold all results.  @return the number of relevant documents scanned while filling <code>results</code>.
     */

    public int process(final String queries, int offset, final int length,
                       final ObjectArrayList<DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>> results,
                       LongSet onlyDocuments) throws QueryParserException, QueryBuilderVisitorException, IOException {
        ...
        return process(partQuery, offset, length, results, onlyDocuments);
    }
    ...
    public int process(final Query query, final int offset, final int length,
                       final ObjectArrayList<DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>> results,
                       LongSet onlyDocuments) throws QueryBuilderVisitorException, IOException {
        return process(new Query[]{query}, offset, length, results, onlyDocuments);
    }
     ...
     * @param length        the number of results to be added to <code>results</code>
     * @param results       an array list that will hold all results.
     * @param onlyDocuments restrict to a given set of documents
     * @return the number of documents scanned while filling <code>results</code>.
     */
    @SuppressWarnings("unchecked")
    public int process(final Query query[], final int offset, final int length, final ObjectArrayList<DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>> results,
                       LongSet onlyDocuments) throws QueryBuilderVisitorException, IOException {
        ...
        for (int i = 0; i < query.length; i++) {
            ...
            count = scorer != null ?
                    getScoredResults(documentIterator, currOffset, currLength, lastMinScore, results, alreadySeen, onlyDocuments) :
                    getResults(documentIterator, currOffset, currLength, results, alreadySeen, onlyDocuments);
    ...
    private int getScoredResults(final DocumentIterator documentIterator, final int offset, final int length,
                                 final double lastMinScore, final ObjectArrayList<DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>> results,
                                 final LongSet alreadySeen, final LongSet onlyDocuments) throws IOException {
        ...
        if (alreadySeen != null)
            while ((document = scorer.nextDocument()) != END_OF_LIST) {
                if (onlyDocuments != null && !onlyDocuments.contains(document)) continue;
    ...
    private int getResults(final DocumentIterator documentIterator, final int offset, final int length,
                           final ObjectArrayList<DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>> results,
                           final LongSet alreadySeen, final LongSet onlyDocuments) throws IOException {
        long document;
        int count = 0; // Number of not-already-seen documents
        ...
        if (alreadySeen != null)
            while ((document = documentIterator.nextDocument()) != END_OF_LIST) {
                ...
                if (count >= offset && count < offset + length)
                    results.add(new DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>>(document, -1));
                count++;
            }
        else if (length != 0)
            while ((document = documentIterator.nextDocument()) != END_OF_LIST) {
