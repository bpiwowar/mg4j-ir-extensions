 ...
 * 
 * <p>Term {@linkplain #occurrences occurrencies} and {@linkplain #count counts} are stored
 ...
 * fill those contaning {@linkplain #occurrences occurrencies} and {@linkplain #indexNumber index numbers}.
...
public class CounterSetupVisitor extends AbstractDocumentIteratorVisitor {
	...
	public CounterSetupVisitor prepare() {
		...
		occurrences = new long[ numberOfPairs ];
	...
	public Boolean visit( final IndexIterator indexIterator ) throws IOException {
		...
		if ( id != -1 ) {
			...
			// BIG HACK STARTS //
			try {
				Class<?> aClass = indexIterator.getClass();
				Field field = null;
				while (field == null) {
					try {
						field = aClass.getDeclaredField("occurrency");
				...
				if (field == null) {
					throw new AssertionError("Cannot find the total number of occurrencies in class " + indexIterator.getClass());
				}

				field.setAccessible(true);
				this.occurrences[ id ] = (Long)field.get(indexIterator);
	 ...
	 * 
	 * <p>This method is usually called back by a {@link it.unimi.di.big.mg4j.search.visitor.CounterCollectionVisitor} built upon
	...
	public String toString() {
		return "[occurrences = " + Arrays.toString(occurrences) + ", count = " + Arrays.toString( count ) +"]";
	}
}
